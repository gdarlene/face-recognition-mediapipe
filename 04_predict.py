...
ap.add_argument("--min-conf", type=float, default=10.0,
                help="Minimum confidence percentage to accept (default: 10)")
ap.add_argument("--camera", type=int, default=1,
                help="Camera index (default 1)")
ap.add_argument("--image", type=str,
                help="If set, run on a single image instead of webcam.")
args = ap.parse_args()
...
if not CASCADE_PATH.exists():
    sys.exit(f"[Error] Haar cascade not found: {CASCADE_PATH}")
...
if args.image:
    ...
    print(f"[info] Image: {img_path.name} | accepted faces: {nfaces}")
    cv2.waitKey(0)
...
while True:
    ...
    hud = f"Accepted: {nfaces}  FPS: {fps:.1f}  thr={args.threshold}  minConf={args.min_conf}%"
